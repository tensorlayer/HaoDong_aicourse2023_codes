...
from tensorlayerx.vision import transforms, load_image
...
class CELEBA(Dataset):
    ...
    def __init__(self):
        images_dir = r'data/celebA/img_align_celeba'
        self.images_path = [os.path.join(images_dir, file) for file in os.listdir(images_dir)]

    def __getitem__(self, idx):
        image = cv2.convertScaleAbs(load_image(self.images_path[idx]))
...
def get_celebA(batch_size):
    ...
    celebA = CELEBA()
    trainloader = DataLoader(celebA, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=16)
    return trainloader, celebA.images_path
