...
class WithLoss_D(Module):
    ...
    def forward(self, images, fake):
        ...
        d_loss_real = tlx.losses.sigmoid_cross_entropy(d2_logits, tlx.ones_like(d2_logits).to(d2_logits.device))
        # discriminator: images from generator (fake) are labelled as 0
        d_loss_fake = tlx.losses.sigmoid_cross_entropy(d_logits, tlx.zeros_like(d_logits).to(d_logits.device))
...
class WithLoss_G(Module):
    ...
    def forward(self, images, fake):
        d_logits = self.D(self.G(fake))
        g_loss = tlx.losses.sigmoid_cross_entropy(d_logits, tlx.ones_like(d_logits).to(d_logits.device))
        return g_loss
...
def train():
    ...
    net_with_loss_D = WithLoss_D(D.to(device), G.to(device)).to(device)
    net_with_loss_G = WithLoss_G(D.to(device), G.to(device)).to(device)
    ...
    for epoch in range(flags.n_epoch):
        for step, batch_images in enumerate(images_loader):
            ...
            z = tlx.ops.convert_to_tensor(z).to(device)
            g_loss = trainforG(batch_images.to(device), z)
            d_loss = trainforD(batch_images.to(device), z)
